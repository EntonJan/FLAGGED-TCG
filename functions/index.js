...
// schon ein shop/{docId}-Dokument existiert (z.B. "daily" oder "weekly") --
// falls nicht, legt sie es
...
//
// Der eigentliche Schutz hier: kein Client kann shop/{docId} direkt
...
exports.ensureShopRotation = functions.https.onCall(async (data, context) => {
  ...
  const { weekId, offers, docId } = data || {};
  const targetDocId = typeof docId === 'string' && docId ? docId : 'current';
  if (typeof weekId !== 'string' || !weekId) {
    throw new functions.https.HttpsError('invalid-argument', 'weekId/cycleId hat ein ungueltiges Format.');
  }
  if (!Array.isArray(offers) || offers.length === 0 || offers.length > 10) {
    throw new functions.https.HttpsError('invalid-argument', 'Es muessen 1 bis 10 Angebote uebergeben werden.');
  }

  const shopRef = db.collection('shop').doc(targetDocId);

  return db.runTransaction(async (tx) => {
    const snap = await tx.get(shopRef);
    if (snap.exists && snap.data().cycleId === weekId) {
      // Fuer diesen Zyklus existiert schon eine Rotation -- nichts tun,
...
exports.purchaseCard = functions.https.onCall(async (data, context) => {
  ...
  const targetDocId = (data && typeof data.docId === 'string' && data.docId) ? data.docId : 'current';
